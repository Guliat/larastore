<div class="columns is-multiline has-text-centered is-mobile">
    ...
    <div class="column is-11 has-background-dark has-text-white is-size-6 has-text-centered">ИЗТИЧАЩИ ПРОМОЦИИ</div>
    <div class="column is-12"></div>
    <?php $promo_products = App\Promotion::where('is_active', '=', 1)->orderBy('end')->paginate(6); $today = date('Y-m-d'); ?>
            ...
            <a href="{{route('slug', $pp->product->slug)}}">
                <div class="card is-shadowless has-ribbon">
                    <div class="ribbon is-dark engagement" style="border-radius: 15%;">
                        <?php $days = strtotime($pp->end) - strtotime($today); ?>
                        <div class="p-t-5"> {{ date('d', $days) }} @if(date('d', $days) == 1) DEH @else DHU @endif </div>
                    </div>
                    <div>
